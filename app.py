...
def first_vis(data):
    ...
    colors = ['#d62728', '#b3baba', '#008f94', '#98df8a']  # colors for the year ranges
    ...
    for column in avg_popularity_genre.columns:
        if column != 'set()':
                bar = go.Bar(
                    ...
                    name=column,
                    marker=dict(color='#008f94', line=dict(color='black', width=1))
                )
                bars.append(bar)
...
st.write("This visualization displays the average popularity of songs in different countries. Each country on the map is color-coded to reflect the average popularity of songs by artists from that country. Warm colors like red indicate lower popularity, while cool colors like blue represent higher popularity. White color indicates that there are no songs available from that country. By hovering over a country, you can discover the specific average popularity of songs by artists from that country. Feel free to explore the map by dragging and using the zoom controls to focus on specific areas of interest.")
second_vis(map_data)
st.subheader('How has the popularity of different genres changed over time?')
st.write("Explore the popularity of different music genres over the years. The graph displays the average popularity of the selected genre across different years. The height of each bar represents the average popularity level of the genre in this year, where higher values indicate greater popularity.Select the desired feature from the scroll-down bar to explore the distribution of its popularity over the years.")
third_vis(data)
